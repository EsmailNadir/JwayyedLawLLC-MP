    ...
    <>
      ...
      <section className="py-16 sm:py-24 bg-gradient-to-b from-gray-50 to-white relative">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {/* Article grid — equal-height cards */}
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-8 sm:items-stretch">
            {filteredPosts.map((post, idx) => (
              <article
                key={post.slug}
                className="group flex flex-col h-full min-h-[420px] bg-white rounded-2xl border border-gray-100 overflow-hidden shadow-sm hover:shadow-xl hover:shadow-gray-200/50 transition-all duration-500 hover:-translate-y-1"
                style={{
              ...
              >
                ...
                {/* Content — flex to fill, consistent sizing */}
                <div className="flex flex-col flex-1 p-6 sm:p-7 min-h-0">
                  <h3 className="font-['Playfair_Display',_'Georgia',_serif] text-xl font-bold text-gray-900 mb-3 leading-snug line-clamp-2 min-h-[3.25rem] group-hover:text-[#b87333] transition-colors duration-300">
                    <Link href={`/our-law-firm/blog/${post.slug}`}>
                      {post.title}
                    </Link>
                  </h3>
                  <p className="font-['DM_Sans',_'Helvetica_Neue',_sans-serif] text-gray-500 text-sm leading-relaxed mb-5 line-clamp-3 min-h-[4rem]">
                    {post.excerpt}
                  </p>
